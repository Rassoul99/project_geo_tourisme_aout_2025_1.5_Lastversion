...
def generate_recommendations(df, min_rating=4.0, stay_duration=1, time_preferences=None, weather=None):
    """
    Génère des recommandations optimisées avec:
    - 1 hôtel par jour
    - 1 restaurant et 1 activité par créneau horaire
    ...
    for day in range(1, stay_duration + 1):
        ...
        for time_slot in time_preferences:
            if not restaurants.empty and r_idx < len(restaurants):
